...
def train(textData, LM, i2v=None, model_path=args['rootDir'] + '/chargemodel_LSTM_IB_GAN.mdl', print_every=50, plot_every=10,
    ...
    train_data = list(beer_reader('./data/beer/reviews.aspect'+str(args["aspect"])+'.train.txt.gz', aspect=args["aspect"], max_len=500))
    test_data = beer_annotations_reader('./data/beer/annotations.json', aspect=args['aspect'] )
    start = time.time()
    plot_losses = []
