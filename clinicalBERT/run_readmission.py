...
def main():
    ...
    parser.add_argument("--classifier_3layer",
                        default=False,
                        action='store_true',
                        help="Whether to run with 3 layer classifier on top.")
    ...
    # tokenizer = BertTokenizer.from_pretrained('./model/biobert_pretrain_output_all_notes_150000/')
    ...
    if args.classifier_3layer:
        model = BertForSequenceClassification_3layer.from_pretrained(args.bert_model, 1)
    else:
        model = BertForSequenceClassification.from_pretrained(args.bert_model, 1)

    print(f'the model loaded is: {model}')
    if args.fp16:
        model.half()
    model.to(device)
    ...
    if args.do_train:
        ...
        for epo in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            val_features = convert_examples_to_features(
                val_examples, label_list, args.max_seq_length, tokenizer)
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(val_dataloader):
                ...
                with torch.no_grad():
                    tmp_val_loss, temp_logits = model(input_ids, segment_ids, input_mask, label_ids)
        ...
        training_stats_df.to_csv(f'{args.output_dir}_training_val_metrics.csv', index=False)

        fig1 = plt.figure()
        plt.plot(train_loss_history)
        fig1.savefig(f'{args.output_dir}/batchloss_{args.readmission_mode}_{args.date}.png', dpi=fig1.dpi)
